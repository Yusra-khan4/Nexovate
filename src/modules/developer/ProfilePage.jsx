...
export default function ProfilePage() {
  ...
  const labelStyles = "block text-xs font-bold text-gray-900 dark:text-black tracking-wide mb-1.5 transition-colors duration-300";
  const inputStyles = "w-full bg-white dark:bg-white border border-gray-300 dark:border-gray-300 rounded-[5px] dark:rounded-[6px] py-2.5 px-4 text-xs text-gray-900 dark:text-gray-900 placeholder-gray-400 dark:placeholder-gray-400 focus:outline-none focus:border-[#DC6B0F] dark:focus:border-[#0284c7] transition-colors duration-300 font-medium shadow-sm disabled:opacity-90";
  ...
  return (
    <div className="flex flex-col min-h-screen py-10 px-4 max-w-5xl mx-auto w-full font-['Raleway',sans-serif] antialiased">
      ...
      <div className="w-full mb-8 text-left">
        ...
        <p className="text-gray-600 dark:text-gray-200 text-xs font-medium">Configure metrics and infrastructure profile specifications.</p>
      </div>

      <div className="w-full flex justify-center items-start flex-1">
        
        {/* OUTER GLASSMORPHIC BORDER WRAPPER FOR DARK MODE */}
        <div className="w-full max-w-2xl dark:p-6 dark:bg-white/10 dark:backdrop-blur-2xl dark:border dark:border-white/15 dark:rounded-[12px] dark:shadow-[0_25px_60px_-15px_rgba(0,0,0,0.5)]">
          
          {/* MAIN PROFILE CARD */}
          <div className="w-full bg-[#FFF6E9] dark:bg-[#EFEEEA] border border-black/5 dark:border-transparent p-8 rounded-[12px] dark:rounded-[12px] shadow-[0_25px_60px_-15px_rgba(0,0,0,0.12)] dark:shadow-none backdrop-blur-xl transition-all duration-300">
            
            {errorMessage && (
              <div className="mb-5 p-3 bg-red-100 dark:bg-red-100 border border-red-500/20 text-red-700 dark:text-red-700 text-xs font-semibold rounded-[5px]">{errorMessage}</div>
            )}
            ...
            <form onSubmit={handleUploadSubmit} className="space-y-6">
              ...
              <div className="flex flex-col items-center mb-4">
                <button type="button" onClick={isEditing ? triggerFileInput : undefined} className={`focus:outline-none flex flex-col items-center ${isEditing ? 'cursor-pointer group' : 'cursor-default'}`}>
                  <div className="w-16 h-16 rounded-full overflow-hidden border-2 border-[#DC6B0F] dark:border-transparent shadow-md relative mb-1 dark:bg-[#1D61E7] flex items-center justify-center">
                    {profile.avatar ? (
                      <img src={profile.avatar} alt="Avatar" className="w-full h-full object-cover" />
                    ) : (
                      <div className="w-full h-full bg-gradient-to-br from-[#F2A508] to-[#BD1C22] dark:bg-none dark:bg-[#1D61E7] flex items-center justify-center text-white font-black text-lg">
                  ...
                  <span className="text-[11px] text-gray-600 dark:text-black font-bold tracking-wide">
                    {isEditing ? 'Edit' : 'Edit'}
                  </span>
                </button>
              </div>
              ...
              <div className="text-left space-y-5 pt-2">
                <h3 className="text-lg font-black tracking-tight text-[#DC6B0F] dark:text-[#0B7EB5]">Skills</h3>
                
                {Object.entries(skillCategories).map(([category, skills], index) => (
                  <div key={category} className="space-y-2">
                    <h4 className="text-xs font-black tracking-wide text-gray-900 dark:text-gray-900 uppercase opacity-80">{category}</h4>
                        ...
                        return (
                          <button
                            ...
                            className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all flex items-center gap-1.5 border select-none ${
                              isSelected 
                                ? 'bg-black text-white dark:bg-[#111111] dark:text-white border-transparent shadow-sm' 
                    ...
                    {/* Divider line between categories in dark mode */}
                    {index < Object.keys(skillCategories).length - 1 && (
                      <div className="hidden dark:block pt-2 border-b border-gray-300/70" />
                    )}
                  </div>
                ))}
              </div>
              ...
              <div className="text-left space-y-3 pt-2 dark:border-t dark:border-gray-300/70">
                    ...
                    <div key={index} className="flex gap-2 items-center">
                      ...
                      {isEditing && (
                        <button
                          ...
                          className="p-2.5 bg-red-100 dark:bg-red-100 text-red-600 dark:text-red-600 rounded-[5px] hover:brightness-95 transition-all cursor-pointer shadow-sm"
                        >
                          <Trash2 size={14} />
                        </button>
                      )}
                ...
                {isEditing && (
                  <button
                    ...
                    className="mt-1 flex items-center gap-1.5 text-xs font-bold text-gray-900 dark:text-gray-900 bg-white/60 dark:bg-white border border-black/10 dark:border-gray-300 rounded-[5px] py-1.5 px-3 hover:bg-white transition-all cursor-pointer shadow-sm"
                  >
              ...
              <div className="text-left space-y-4 pt-2 border-t border-black/5 dark:border-gray-300/70">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
              ...
              <div className="w-full flex justify-start gap-2.5 pt-4 dark:border-t dark:border-gray-300/70">
                {isEditing ? (
                  <>
                    <button 
                      type="submit" 
                    ...
                    <button 
                      ...
                      className="bg-white dark:bg-white text-black dark:text-black font-bold text-xs py-2.5 px-4 rounded-[5px] border border-black/10 dark:border-gray-300 hover:bg-gray-50 dark:hover:bg-gray-100 active:scale-[0.98] transition-all cursor-pointer"
                    >
                      Cancel
                    </button>
                  </>
                  ...
                  <button 
                    ...
                    className="bg-gradient-to-r from-[#F2A508] via-[#DC6B0F] to-[#BD1C22] dark:bg-gradient-to-r dark:from-[#F2A508] dark:via-[#DC6B0F] dark:to-[#BD1C22] text-white dark:text-white font-extrabold text-xs px-5 py-2.5 rounded-[5px] shadow-md border border-black/10 dark:border-transparent flex items-center gap-2 hover:brightness-105 active:scale-[0.98] transition-all cursor-pointer"
                  >
